...
from app.service.admin.model_catalog_service import ModelCatalogService  # noqa: E402
...
async def test_invalidate_cache_calls_scan_and_delete():
    ...
    with patch(
        "app.service.admin.model_catalog_service.get_cache_redis",
...
async def test_create_vendor_invalidates_cache(mock_super_admin):
    ...
    with patch(
        "app.service.admin.model_catalog_service.ModelVendorRepository",
        return_value=repo_mock,
    ), patch(
        "app.service.admin.model_catalog_service.AdminAuditService.record",
        new_callable=AsyncMock,
    ), patch.object(
        ModelCatalogService, "_invalidate_cache", new_callable=AsyncMock,
    ) as invalidate_mock:
        from app.schema.admin.model_catalog import ModelVendorCreate
...
async def test_invalidates_on_all_writes(mock_super_admin):
    ...
    commit returns.
    """
    from app.schema.admin.model_catalog import (
        ModelCategoryCreate,
        ModelCategoryUpdate,
        ModelVendorCreate,
        ModelVendorUpdate,
    ...
    async def _run_create_category():
        ...
        with patch(
            "app.service.admin.model_catalog_service.ModelCategoryRepository",
            return_value=repo,
        ), patch(
            "app.service.admin.model_catalog_service.AdminAuditService.record",
            new_callable=AsyncMock,
    ...
    async def _run_disable_model():
        ...
        with patch(
            "app.service.admin.model_catalog_service.ModelCatalogRepository",
            return_value=repo,
        ), patch(
            "app.service.admin.model_catalog_service.AdminAuditService.record",
            new_callable=AsyncMock,
