...
Ported from router-service/src/services/anthropic_convert.py.
"""
...
def anthropic_to_openai_request(
    ...
    if getattr(request, "metadata", None) and request.metadata.get("user_id"):
...
# PLACEHOLDER_ANTHROPIC_CONVERT_PART2


def build_anthropic_native_params(request: Any) -> dict[str, Any]:
...
# PLACEHOLDER_ANTHROPIC_STREAM_CONVERTER
...
class AnthropicStreamConverter:
    ...
    def _emit_block_start_text(self) -> str:
        event = _sse_event("content_block_start", {
            "type": "content_block_start", "index": self._content_index,
    ...
    def _emit_thinking_delta(self, text: str) -> str:
        return _sse_event("content_block_delta", {
            "type": "content_block_delta", "index": self._content_index,
    ...
    # PLACEHOLDER_ANTHROPIC_STREAM_CONVERTER_PART2
    ...
    def convert_chunk(self, chunk_dict: dict[str, Any]) -> str:
        ...
        if not choices:
            if chunk_usage and self._started and not self._finished:
                events += self._emit_finish("end_turn")
            return events
