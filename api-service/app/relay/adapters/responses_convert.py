...
Ported from router-service/src/services/responses_convert.py.
"""
...
# PLACEHOLDER_RESPONSES_STREAM_CONVERTER
...
class ResponsesStreamConverter:
    ...
    def _emit_created(self) -> str:
        self._started = True
        resp_obj = {
            "id": self._resp_id, "object": "response",
            "created_at": self._created_at, "status": "in_progress",
            "model": self._model, "output": [], "usage": None,
        }
    ...
    def _emit_output_item_added_message(self) -> str:
        ...
        item = {"type": "message", "id": self._msg_id, "status": "in_progress", "role": "assistant", "content": []}
        events = _sse_event("response.output_item.added", {
            "type": "response.output_item.added", "output_index": 0, "item": item,
        })
        events += _sse_event("response.content_part.added", {
            "type": "response.content_part.added", "item_id": self._msg_id,
    ...
    def _emit_text_delta(self, text: str) -> str:
        ...
        return _sse_event("response.output_text.delta", {
            "type": "response.output_text.delta", "item_id": self._msg_id,
            "output_index": 0, "content_index": 0, "delta": text,
        })

    def _emit_text_done(self) -> str:
        events = _sse_event("response.output_text.done", {
            "type": "response.output_text.done", "item_id": self._msg_id,
            "output_index": 0, "content_index": 0, "text": self._text_content,
        })
        events += _sse_event("response.content_part.done", {
            "type": "response.content_part.done", "item_id": self._msg_id,
            "output_index": 0, "content_index": 0,
            "part": {"type": "output_text", "text": self._text_content, "annotations": []},
        })
        events += _sse_event("response.output_item.done", {
            "type": "response.output_item.done", "output_index": 0,
            "item": {"type": "message", "id": self._msg_id, "status": "completed",
                     "role": "assistant",
                     "content": [{"type": "output_text", "text": self._text_content, "annotations": []}]},
        })
        return events

    # PLACEHOLDER_RESPONSES_STREAM_CONVERTER_PART2
    ...
    def _emit_tool_added(self, tc_index: int, call_id: str, name: str) -> str:
        ...
        self._tool_calls[tc_index] = {
            "id": fc_id, "call_id": call_id, "name": name,
    ...
    def _emit_tool_args_delta(self, tc_index: int, delta: str) -> str:
        ...
        return _sse_event("response.function_call_arguments.delta", {
            ...
            "item_id": tc.get("id", ""), "call_id": tc.get("call_id", ""),
            "output_index": tc.get("output_index", 0), "delta": delta,
        })
    ...
    def _emit_tool_done(self, tc_index: int) -> str:
        ...
        events = _sse_event("response.function_call_arguments.done", {
            ...
            "item_id": tc.get("id", ""), "output_index": output_index,
            "arguments": tc.get("arguments", ""),
        })
        events += _sse_event("response.output_item.done", {
            "type": "response.output_item.done", "output_index": output_index,
            "item": {"type": "function_call", "id": tc.get("id", ""),
                     "call_id": tc.get("call_id", ""), "name": tc.get("name", ""),
                     "arguments": tc.get("arguments", ""), "status": "completed"},
        })
        return events
    ...
    def _emit_completed(self) -> str:
        ...
        if self._text_started:
            output.append({
                "type": "message", "id": self._msg_id, "status": "completed",
                "role": "assistant",
                "content": [{"type": "output_text", "text": self._text_content, "annotations": []}],
            })
        for tc in self._tool_calls.values():
            output.append({
                "type": "function_call", "id": tc.get("id", ""),
                "call_id": tc.get("call_id", ""), "name": tc.get("name", ""),
                "arguments": tc.get("arguments", ""), "status": "completed",
            })
        if not output:
        ...
        resp_obj = {
            "id": self._resp_id, "object": "response",
            "created_at": self._created_at, "status": resp_status,
            "model": self._model, "output": output, "usage": self._usage,
        }
