...
from app.relay.auth import ValidatedApiKey
from app.relay.call_log_writer import create_call_log
...
from app.relay.routing import RoutingError, route_and_resolve
from app.relay.upstream_dispatch import dispatch
...
class CallLifecycle:
    ...
    async def _re_resolve(self, tried_slugs: set[str]) -> None:
        config_cache = get_routing_config_cache()
        channel_selector = get_channel_selector()
        from app.relay.upstream import resolve_model_channel_target
    ...
    def _build_stream_response(self) -> StreamingResponse:
        from app.relay.lifecycle.stream import stream_events, stream_native_anthropic
