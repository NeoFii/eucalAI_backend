...
from app.common.infra.db.repository import BaseRepository
from app.common.infra.db.query import ListParams, PaginatedResult
from app.common.utils.timezone import now
from app.model import User, UserSession, EmailVerificationCode
...
class UserRepository(BaseRepository[User]):
    ...
    async def revoke_all_sessions(self, user_id: int) -> int:
        ...
        Phase 5 / Plan 05-03 T-5-07: admin reset-password must invalidate every
        currently-active session for the target user so old refresh tokens are
        no longer usable. Returns the number of sessions revoked.

        Additive (does not change existing call sites). Mirrors the
        AuthService._revoke_all_user_sessions helper but exposes the
        invalidation at the repository layer for direct use by
        AdminEndUserService.reset_user_password.
        """
        sessions = await self.list_active_sessions_for_user(user_id)
        revoked = 0
        timestamp = now()
        for session_obj in sessions:
            session_obj.revoked_at = timestamp
            revoked += 1
        return revoked
