"""Validate user-service runtime environment before startup."""
...
def validate_environment(environ: Mapping[str, str] | None = None) -> ValidationResult:
    ...
    if not _get_env(env, "USER_DATABASE_URL"):
        result.errors.append("Missing required: USER_DATABASE_URL")
    ...
    if _get_env(env, "DATABASE_URL"):
        result.warnings.append("DATABASE_URL is set but ignored; use USER_DATABASE_URL")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Validate user-service environment")
