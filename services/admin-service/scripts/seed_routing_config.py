...
Reads the existing config/runtime_config.json, creates provider
credentials (encrypted) and a v1 routing config, then publishes it.

Usage:
    uv run seed-routing-config
    uv run seed-routing-config --config path/to/runtime_config.json
...
DEFAULT_CONFIG_PATH = SERVICE_ROOT / "config" / "runtime_config.json"
