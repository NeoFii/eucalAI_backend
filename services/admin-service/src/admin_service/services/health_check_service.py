...
from admin_service.services.pool_service import _extract_balance
from common.utils.crypto import decrypt_api_key

logger = logging.getLogger("admin_service.health_check")
    ...
    async def run_health_checks(db: AsyncSession) -> None:
        ...
        for pool in pools:
            ...
            for account in accounts:
                ok = await HealthCheckService._check_account(db, pool, account)
                checked += 1
                if not ok:
                    errors += 1
        ...
        await db.commit()
        logger.info("health check complete: checked=%d errors=%d", checked, errors)
