...
Ported from router-service/src/services/inference_client.py.
Decision D-19: Retain HTTP call to inference-service on GPU server.
Threat T-06-10: X-Inference-Secret header never logged.
"""
...
class ClassifyResult:
    """Result of an inference-service classify call."""
...
class InferenceClient:
    ...
    def _check_circuit_breaker(self) -> ClassifyResult | None:
        """Return a circuit-open result if CB is tripped, else None."""
        if self._cb_failures >= self._cb_threshold:
            ...
            # Cooldown elapsed — reset and allow attempt
            self._cb_failures = 0
        return None
    ...
    async def classify(
        ...
        """Call inference-service /internal/v1/classify with retry + circuit breaker."""
        ...
        for attempt in range(attempts):
            try:
                ...
                if resp.status_code < 400:
                    ...
                    return ClassifyResult(
                        success=True, data=resp.json(), http_status=resp.status_code
                    )
    ...
    def _parse_error_response(resp: httpx.Response) -> tuple[str, str]:
        """Extract error_code and message from response body."""
        try:
            body = resp.json()
...
__all__ = ["ClassifyResult", "InferenceClient"]
