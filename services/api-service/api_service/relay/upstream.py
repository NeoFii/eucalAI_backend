"""Upstream model resolution — ported from router-service/src/services/upstream.py.

Pure functions for resolving logical model names to concrete upstream targets,
validating URLs against SSRF blocklist, and stripping think tags.
"""
...
def _validate_upstream_url(url: str) -> None:
    """Validate that an upstream URL does not target blocked/private hosts."""
    parsed = urlparse(url)
...
def normalize_api_base(value: str) -> str:
    """Normalize an API base URL by stripping trailing path segments."""
...
def resolve_model_provider_target(
    ...
    """Resolve a logical model to its provider target (single-provider mode)."""
...
def resolve_model_channel_target(
    ...
    *,
    ...
    """Resolve a model to a specific channel from the pool via channel_selector."""
