"""Runtime configuration normalization — ported from router-service/src/utils/runtime_config.py.

Defines constants locally (D-11 discretion) and provides normalize_runtime_config()
for transforming raw routing config dicts into a validated, standardized structure.
"""
...
# ── Constants (defined locally per D-11, no import from router-service) ──────

DEFAULT_ROUTER_ALIAS = "auto"
FIVEWAY_ROUTE_ORDER: tuple[str, ...] = ("纠错", "工具调用", "通用任务", "任务拆解", "编程")
FIVEWAY_DEFAULT_WEIGHTS: Dict[str, float] = {k: 1.0 for k in FIVEWAY_ROUTE_ORDER}
...
def parse_score_bands(raw: str) -> List[Tuple[float, float, int]]:
    """Parse score band string like '0-3:5,3-5:4,5-7:3,7-9:2,9-10:1'."""
...
def build_default_runtime_config() -> Dict[str, Any]:
    """Return a default runtime config dict with empty providers/channels."""
    return {
...
def _coerce_default_user_rpm(value: Any) -> int | None:
    """Coerce admin-supplied RPM value to a positive int, else None.

    Shared coercer for default_user_rpm and system_rpm_cap since they have
    identical semantics: None means 'no override'. Bad values silently
...
def normalize_runtime_config(raw: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    if tuple(route_order) != FIVEWAY_ROUTE_ORDER:
    ...
    router_alias = (
        str(raw.get("router_alias", base["router_alias"])).strip() or DEFAULT_ROUTER_ALIAS
    )
            ...
            for ch in channels:
                ...
                for key in ("channel_slug", "provider_slug", "api_key", "api_base", "upstream_model"):
                    if key not in ch or not str(ch[key]).strip():
                        raise ValueError(
                            f"model_channels[{model_name}] channel missing {key}"
                        )
                validated.append({
                    ...
                    "input_price_per_million": int(ch.get("cost_input_per_million", 0)),
                    "output_price_per_million": int(ch.get("cost_output_per_million", 0)),
                    "cached_input_price_per_million": int(
                        ch.get("cost_cached_input_per_million", 0)
                    ),
    ...
    # Model prices (user-facing, from model_catalog)
