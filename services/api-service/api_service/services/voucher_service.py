...
from api_service.common.core.exceptions import UserNotFoundException, ValidationException
from api_service.common.infra.db.query import ListParams, PaginatedResult
from api_service.common.utils.timezone import now, to_shanghai_naive
from api_service.models import BalanceTransaction, VoucherRedemptionCode
...
class VoucherService:
    ...
    def normalize_code(raw_code: str) -> str:
        # Pitfall 10: single source of truth for normalization — strip + lower.
    ...
    async def get(db: AsyncSession, code_id: int) -> VoucherRedemptionCode:
        code = await VoucherRepository(db).get_by_id(code_id)
        if code is None:
    ...
    async def list_for_admin(
        ...
        *,
        ...
        return await VoucherRepository(db).list_for_admin(params, status=status)
    ...
    async def list_user_redemptions(
        ...
        *,
        ...
        return await VoucherRepository(db).list_for_user_redemptions(
            user_id=user_id,
            params=params,
        )
    ...
    async def disable(
        ...
        *,
        ...
        _ = operator_id
        code = await VoucherRepository(db).get_by_id(code_id, for_update=True)
        if code is None:
            raise ValidationException(detail="代金券兑换码不存在")
    ...
    async def redeem_code(
        ...
        *,
        ...
        code_repo = VoucherRepository(db)
        ...
        # SELECT FOR UPDATE row lock on user — wallet mutation invariant.
        user = await UserRepository(db).get_by_id(user_id, for_update=True)
        if user is None:
            raise UserNotFoundException()

        billing_repo = BillingRepository(db)
        # ref_id idempotency: a duplicate redeem attempt for the same voucher code
        # must NOT double-credit the user. The exists check short-circuits before
        # any balance mutation.
        if await billing_repo.exists_by_ref(
            tx_type=BalanceTransaction.TYPE_VOUCHER_REDEEM,
