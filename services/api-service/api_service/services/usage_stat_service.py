"""Usage-stat aggregation and query helpers for api-service."""
...
from api_service.models import ApiCallLog, UsageStat
...
class UsageStatService:
    ...
    async def upsert_from_log(db: AsyncSession, log: ApiCallLog) -> None:
        ...
        Pitfall 8: requests tagged ``error_code == 'invalid_model'`` are skipped
        here. The repository methods used elsewhere already embed the same
        filter at the SQL layer; this is the per-row guard for the upsert path.
        """
        ...
        for api_key_id in (log.api_key_id, None):
            bucket = await repo.stat_get_bucket(
            ...
            if bucket is None:
                ...
                # No per-row helper on BillingRepository — use the session add directly.
                repo.session.add(bucket)
                await db.flush()
    ...
    async def aggregate_hour(db: AsyncSession, stat_hour: datetime) -> None:
        ...
        Pitfall 8: ``BillingRepository.stat_list_logs_for_hour`` already filters
        ``error_code == 'invalid_model'`` at the SQL layer, so this method does
        not need to re-filter.
        """
        next_hour = stat_hour + timedelta(hours=1)
        repo = BillingRepository(db)
        logs = await repo.stat_list_logs_for_hour(stat_hour, next_hour)
        ...
        for (user_id, api_key_id, model_name), metrics in buckets.items():
            existing = await repo.stat_get_bucket(
                user_id=user_id,
                api_key_id=api_key_id,
                model_name=model_name,
                stat_hour=stat_hour,
    ...
    async def get_user_stats(
        ...
        return await BillingRepository(db).stat_get_user_stats(
            user_id=user_id,
            start=start,
            end=end,
            model_name=model_name,
    ...
    async def get_all_stats(
        ...
        return await BillingRepository(db).stat_get_all_stats(
            start=start,
            end=end,
            user_id=user_id,
            model_name=model_name,
    ...
    async def list_usage_logs(
        ...
        *,
        ...
        return await BillingRepository(db).stat_list_usage_logs(
            params=params,
            user_id=user_id,
            api_key_id=api_key_id,
            model_name=model_name,
    ...
    async def get_usage_analytics(
        ...
        *,
        ...
        logs = await BillingRepository(db).stat_list_analytics_logs(
