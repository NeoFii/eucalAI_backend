...
from api_service.common.observability import REQUEST_ID_HEADER, get_request_id, log_event
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def validation_exception_handler(request: Request, exc: RequestValidationError):
        """Log and return request validation errors."""
        log_event(
            logger,
            logging.WARNING,
            "validation_error",
    ...
    async def unhandled_exception_handler(request: Request, exc: Exception):
        """Log uncaught exceptions without exposing internals to clients."""
        log_event(
            logger,
            logging.ERROR,
            "unhandled_exception",
