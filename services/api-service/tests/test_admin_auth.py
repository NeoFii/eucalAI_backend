...
async def test_login_sets_cookies(client: AsyncClient, mock_admin):
    ...
    with patch(
        "api_service.controllers.admin.auth.AdminAuthService.login",
...
async def test_lockout(mock_db, mock_admin):
    ...
    from api_service.common.core.exceptions import InvalidCredentialsException
    from api_service.common.utils.timezone import now
    from api_service.services.admin.auth_service import (
    ...
    with (
        patch(
            "api_service.services.admin.auth_service.AdminUserRepository",
            return_value=mock_repo,
        ),
        patch(
            "api_service.services.admin.auth_service.verify_password_async",
            new=AsyncMock(return_value=False),
        ),
        patch(
            "api_service.services.admin.auth_service.AdminAuditService.record",
            new=AsyncMock(),
        ),
    ):
...
async def test_logout_blacklists(client: AsyncClient, mock_admin):
    ...
    with (
        patch(
            "api_service.services.admin.auth_service.blacklist_token",
            new=AsyncMock(return_value=True),
        ) as mock_blacklist,
        patch(
            "api_service.services.admin.auth_service.get_token_jti",
            side_effect=lambda token: f"jti-{token}",
        ),
        patch(
            "api_service.services.admin.auth_service._remaining_ttl",
            return_value=600,
        ),
    ):
        response = await client.post(
...
async def test_refresh_rotates(client: AsyncClient):
    ...
    with patch(
        "api_service.controllers.admin.auth.AdminAuthService.refresh_access_token",
...
async def test_change_password_invalidates(client: AsyncClient, mock_admin):
    ...
    with (
        ...
        patch(
            "api_service.services.admin.auth_service.check_password_strength",
            return_value=(True, ""),
        ),
        patch(
            "api_service.services.admin.auth_service.hash_password_async",
