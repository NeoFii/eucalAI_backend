...
from api_service.relay.auth import (
    ValidatedApiKey,
    _api_key_cache,
...
async def test_require_api_key_redis_hit():
    ...
    request = _make_request()
    with patch("api_service.relay.auth.get_cache_redis", return_value=mock_redis):
        result = await require_api_key(
            request, authorization=f"Bearer {RAW_KEY}", x_api_key=None
        )
...
async def test_require_api_key_db_fallback():
    ...
    request = _make_request()
    with (
        patch("api_service.relay.auth.get_cache_redis", return_value=mock_redis),
        patch("api_service.relay.auth.get_db_context") as mock_ctx,
        patch(
            "api_service.services.api_key_service.ApiKeyService.validate_by_hash",
...
async def test_require_api_key_invalid_key():
    ...
    from api_service.common.core.exceptions import ApiKeyNotFoundException
    from fastapi import HTTPException
