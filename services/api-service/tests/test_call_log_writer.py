...
from api_service.relay.call_log_writer import (
...
class TestCreateCallLog:
    ...
    async def test_create_call_log_writes_to_db(self):
        ...
        with patch("api_service.relay.call_log_writer.ApiCallLog") as MockLog:
...
class TestUpdateAndSettle:
    ...
    async def test_update_and_settle_updates_then_settles(self):
        ...
        with patch("api_service.relay.billing.RelayBillingService.settle", new_callable=AsyncMock) as mock_settle:
            with patch("api_service.services.balance_service.BalanceService.consume_for_call_log", new_callable=AsyncMock) as mock_consume:
    ...
    async def test_update_and_settle_billing_retry(self):
        ...
        with patch("api_service.relay.billing.RelayBillingService.settle", new_callable=AsyncMock, side_effect=settle_with_failures) as mock_settle:
            with patch("api_service.services.balance_service.BalanceService.consume_for_call_log", new_callable=AsyncMock):
                await _write_call_log_update_and_settle(
    ...
    async def test_update_and_settle_billing_all_retries_fail(self, caplog):
        ...
        with caplog.at_level(logging.ERROR):
            with patch("api_service.relay.billing.RelayBillingService.settle", new_callable=AsyncMock, side_effect=Exception("Redis down")):
                with patch("api_service.services.balance_service.BalanceService.consume_for_call_log", new_callable=AsyncMock, side_effect=Exception("DB down")):
                    await _write_call_log_update_and_settle(
