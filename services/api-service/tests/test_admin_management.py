...
- `test_account_service_renamed` (Pitfall 3): the symbol
  `AdminAccountService` exists at `api_service.services.admin.account_service`
...
from api_service.common.core.exceptions import AdminConflictException  # noqa: E402


def test_account_service_renamed():
    """Pitfall 3: `AdminAccountService` exists; `AdminManagementService` does NOT."""
    from api_service.services.admin import account_service as m
...
async def test_create_admin_email_conflict(mock_super_admin):
    ...
    with patch(
        "api_service.services.admin.account_service.AdminUserRepository",
        return_value=repo,
    ):
