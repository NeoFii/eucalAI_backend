...
async def test_list_no_http(mock_user_obj, mock_super_admin_obj):
    ...
    with patch(
        "api_service.services.admin.admin_user_service.UserRepository"
    ) as MockRepo:
...
async def test_topup_atomic_with_audit(mock_user_obj, mock_super_admin_obj):
    ...
    with patch(
        ...
        "api_service.services.admin.admin_user_service.BalanceService"
    ) as MockBalance, patch(
        "api_service.controllers.admin.users.AdminAuditService"
    ) as MockAudit:
        repo_instance = MagicMock()
...
async def test_reset_password_revokes_sessions(mock_user_obj, mock_super_admin_obj):
    ...
    with patch(
        ...
        "api_service.services.admin.admin_user_service.hash_password_async",
        new=AsyncMock(return_value="$2b$12$newhash"),
    ), patch(
        "api_service.services.admin.admin_user_service.AuthService"
    ) as MockAuth, patch(
        "api_service.controllers.admin.users.AdminAuditService"
    ) as MockAudit:
        repo_instance = MagicMock()
