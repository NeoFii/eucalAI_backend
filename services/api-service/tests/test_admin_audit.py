...
from api_service.services.admin.audit_service import AdminAuditService  # noqa: E402


def test_audit_logs_router_mounted():
    """Router exposes /meta, /, /action-definitions/{code} (3 endpoints)."""
    from api_service.controllers.admin.audit_logs import router
...
async def test_meta_returns_shape(mock_super_admin):
    ...
    with patch.object(
        ...
        import api_service.services.admin.audit_service as svc
...
async def test_list_filters(mock_super_admin):
    ...
    with patch(
        "api_service.services.admin.audit_service.AdminUserRepository",
        return_value=repo_mock,
    ), patch(
        "api_service.services.admin.audit_service.AuditLogRepository",
...
def test_run_health_checks_is_callable():
    ...
    from api_service.core.jobs import run_health_checks
...
def test_no_safe_audit_commit_in_audit_logs_controller():
    ...
    from api_service.controllers.admin import audit_logs
