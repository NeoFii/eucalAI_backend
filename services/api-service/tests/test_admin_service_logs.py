...
async def test_local_only(mock_super_admin_obj):
    ...
    with patch(
        "api_service.services.admin.service_logs_service.get_ring_buffer",
        return_value=mock_buf,
    ), patch(
        "api_service.services.admin.service_logs_service.get_internal_json",
...
async def test_partial_on_failure(mock_super_admin_obj):
    ...
    from api_service.core.policies import require_super_admin
    from api_service.common.internal import InternalServiceError
...
def test_remote_services_only_inference():
    ...
    from api_service.services.admin.service_logs_service import _REMOTE_SERVICES
