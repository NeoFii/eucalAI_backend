...
后端共 **3 个 MySQL 库**，每个库由一个服务独占写入。router-service 是 ML 推理服务，**没有库**。
...
ID 惯例：所有业务表 `id` 是 `BigInteger`，由 `SnowflakeIdMixin` 生成雪花 ID；对外暴露字段叫 `uid` / `slug` / `code` / `session_id` 等（看语义）。审计日志、连接表、验证码类、字典表以及 testing 库的大部分业务表用 `autoincrement=True` 的普通 BIGINT。
...
|---|---|---|---|
| `id` | BIGINT PK | AUTO_INCREMENT | — |
| `vendor_id` | BIGINT FK→model_vendors.id | NOT NULL，**未显式 ondelete**（=NO ACTION） | 删 vendor 会被数据库拦住 |
...
约束：`UK (model_id, provider_id)`（同一模型在同一 provider 最多一条），索引 `idx_mpo_model_id` / `idx_mpo_provider_id` / `idx_mpo_is_active` / `idx_mpo_deleted_at`。
...
| `sample_count`, `success_count`, `fail_count` | INT | NOT NULL, DEFAULT 0 | — |
...
约束：`uk_provider_performance_daily_stats (offering_id, probe_region, stat_date)`；索引 `idx_ppds_date`、`idx_ppds_offering_date`。
...
| `status` | VARCHAR(20) | NOT NULL, DEFAULT 'queued' | `queued/running/succeeded/failed/partial` |
...
| `trigger_source` | VARCHAR(20) | NOT NULL, DEFAULT 'manual' | `manual` / `scheduler` |
| `total/completed/succeeded/failed_offerings` | INT | NOT NULL, DEFAULT 0 | 进度计数 |
...
**不是表，是视图**。ORM 里用 `info: {"is_view": True}` 标注，Alembic `create_metadata_objects` 跳过建表，随后用 `create_or_replace_view` 注入视图 DDL。

视图 SQL（见 `migrations/testing_service/versions/20260313_05_testing_baseline.py`）：
...
用途：查询"某 offering × region 最近 N 次成功探测"只要 `WHERE rn <= N`。

---

## 5. 跨库引用总览

三个库之间**没有数据库层面的外键**。以下字段是靠应用层 / HMAC API 维护的"逻辑外键"：
...
- 建表 / 升级统一走：
  ...
  uv run bootstrap-databases        # 一次 3 库全 upgrade
  ```
...
| 模式 | 举例 |
|---|---|
| **CASCADE 删**（子随父消失） | `user_sessions.user_id → users.id`；`user_active_sessions.user_id → users.id`；`user_active_sessions.session_id → user_sessions.session_id`；`model_provider_offerings.{model_id,provider_id} → {models,providers}.id`；`provider_performance_metrics.offering_id → model_provider_offerings.id`；`provider_performance_daily_stats.offering_id → model_provider_offerings.id`；`model_category_map.{model_id,category_id} → {models,model_categories}.id`；`provider_probe_configs.provider_id → providers.id` |
| **SET NULL**（保留残影） | 所有自引用 `admin_users.{created_by,updated_by,password_changed_by}_admin_id`；`invitation_codes.created_by`；`admin_audit_logs.target_admin_id`；`benchmark_jobs.scope_offering_id`；`admin_probe_audit_logs.{offering_id, model_id, provider_id}` |
| **RESTRICT**（拦住硬删） | `admin_audit_logs.actor_admin_id → admin_users.id` |
| **NO ACTION（未显式声明，数据库拦截）** | `models.vendor_id → model_vendors.id` |
| **无 FK（跨库）** | §6 的所有条目 |
...
- `testing.model_categories` / `model_vendors` / `models` / `model_category_map`（复合 PK，无独立 id）/ `providers` / `provider_probe_configs`（PK=provider_id）/ `model_provider_offerings` / `provider_performance_metrics` / `provider_performance_daily_stats` / `benchmark_jobs` / `admin_probe_audit_logs`
- `user.user_active_sessions`（PK=user_id，本身也不是 Snowflake 生成）
- 其余业务表都是 Snowflake（`common/db/base.py::SnowflakeIdMixin`）

**哪些是多对多连接表？**
- `testing.model_category_map`（models × model_categories，复合 PK）

**哪些表是 1:1 关系？**
- `user_active_sessions` ↔ `users`（`user_id` 作 PK）
- `user_active_sessions` ↔ `user_sessions`（`session_id` UNIQUE）
- `provider_probe_configs` ↔ `providers`（`provider_id` 作 PK）

**有视图吗？**
- `testing.provider_metrics_ranked` — 不是表，是 MySQL 8 的 `ROW_NUMBER() OVER (PARTITION BY ...)` 视图，ORM 侧用 `info: {"is_view": True}` 标注，定义在 `migrations/testing_service/versions/20260313_05_testing_baseline.py`。
