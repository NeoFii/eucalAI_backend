...
@pytest.mark.asyncio
async def test_admin_news_publish_can_be_read_via_user_public_news_endpoint(monkeypatch):
    from content_service.api.v1.endpoints import admin_news, news as public_news
    from content_service.schemas import CreateNewsRequest
    from user_service.api.v1.endpoints import news as user_news
    ...
    async def fake_get_published_by_slug(_db, slug):
        return store.get(slug)

    async def fake_list_published(_db, page, page_size):
        del page, page_size
        return list(store.values()), len(store)

    async def fake_public_get_news(slug):
        return await public_news.get_news(slug=slug, db=object())

    async def fake_public_list_news(*, page, page_size):
        return await public_news.list_news(page=page, page_size=page_size, db=object())

    monkeypatch.setattr("content_service.api.v1.endpoints.admin_news.NewsService.create", fake_create)
    monkeypatch.setattr(
        "content_service.api.v1.endpoints.news.NewsService.get_published_by_slug",
        fake_get_published_by_slug,
    )
    monkeypatch.setattr(
        "content_service.api.v1.endpoints.news.NewsService.list_published",
        fake_list_published,
    )
    monkeypatch.setattr(
        "user_service.api.v1.endpoints.news.ContentPublicClientService.get_news",
        fake_public_get_news,
    )
    monkeypatch.setattr(
        "user_service.api.v1.endpoints.news.ContentPublicClientService.list_news",
        fake_public_list_news,
    )

    created = await admin_news.create_news(
    ...
    fetched = await user_news.get_news("e2e-news")
    listed = await user_news.list_news(page=1, page_size=20)

    assert created.data.slug == "e2e-news"
    assert fetched["data"]["slug"] == "e2e-news"
    assert fetched["data"]["title"] == "E2E News"
    assert listed["data"]["items"][0]["slug"] == "e2e-news"
