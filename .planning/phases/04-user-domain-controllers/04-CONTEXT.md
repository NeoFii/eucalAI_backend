...
- **D-09 (回应 O-1):** `/auth/me` 的 `rpm` 字段使用 `settings.DEFAULT_USER_RPM` 常量兜底，Phase 4 不读 system_settings 表/不调 admin。需要在 `ApiServiceSettings` 中新增 `DEFAULT_USER_RPM: int = 20`（与现 user-service `core/config.py` 默认一致，源码真值 = 20）。Phase 5 admin 域接入真正的 system_settings 表读写后可改为读 DB。
